...
export default function CartDrawer({
  ...
  return (
    <>
      ...
      <aside
        className="fixed right-0 top-0 z-50 h-full w-[90%] sm:w-[420px] bg-white text-black shadow-xl pt-6 overflow-y-auto"
        role="dialog" aria-label="Tiroir panier"
      >
        <div className="px-4 sm:px-5">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold">Votre panier</h2>
            <button onClick={onClose} className="rounded-md border px-2 py-1 text-sm hover:bg-gray-100">Fermer</button>
          </div>
              ...
              items.map((it) => (
                <div key={it.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-3">
                    <img src={it.image} alt={it.title} className="h-14 w-14 rounded object-cover" />
                    <div>
          ...
          <div className="mt-5 border-t pt-4 flex items-center justify-between">
            ...
            <div className="text-lg font-semibold">{Number(total || 0).toFixed(2)} $</div>
          </div>
