"use client";
export const dynamic = "force-dynamic"; // évite le prerender SSR pour le panier
...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4 grid gap-8 lg:grid-cols-2">
      <div className="space-y-4">
        <h1 className="text-2xl font-semibold text-white">Informations de paiement</h1>
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input className="rounded-lg px-3 py-2 bg-white/90 text-black" placeholder="Prénom"
              value={form.prenom} onChange={(e)=>setForm({...form, prenom: e.target.value})}/>
            <input className="rounded-lg px-3 py-2 bg-white/90 text-black" placeholder="Nom"
              value={form.nom} onChange={(e)=>setForm({...form, nom: e.target.value})}/>
          </div>
          <input className="rounded-lg px-3 py-2 bg-white/90 text-black w-full" placeholder="Adresse"
            value={form.adresse} onChange={(e)=>setForm({...form, adresse: e.target.value})}/>
          <input className="rounded-lg px-3 py-2 bg-white/90 text-black w-full" placeholder="Ville"
            value={form.ville} onChange={(e)=>setForm({...form, ville: e.target.value})}/>
          <input className="rounded-lg px-3 py-2 bg-white/90 text-black w-full" placeholder="Téléphone"
            value={form.telephone} onChange={(e)=>setForm({...form, telephone: e.target.value})}/>
          <button className="w-full rounded-xl bg-cyan-400 px-5 py-3 text-black font-semibold hover:brightness-110">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between text-lg text-white border-t border-white/10 pt-3">
          <span>Total</span>
